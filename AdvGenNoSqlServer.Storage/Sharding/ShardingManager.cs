...
public class ShardingManager : IShardingManager
{
    ...
    public Task<ShardingClusterStatistics> GetStatisticsAsync(CancellationToken cancellationToken = default)
    {
        ...
        double sumLatency = 0;
        int statsCount = 0;
        var shardStatsList = new List<ShardStatistics>();

        // Iterate exactly once to avoid multiple lock acquisitions,
        // O(N) memory allocations, and redundant linear scans
    ...
    {
        if (_statistics.IsEmpty) return false;
        ...
        double avgLoad = (double)sumLoad / activeCount;
