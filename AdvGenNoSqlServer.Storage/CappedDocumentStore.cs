...
public class CappedDocumentStore : IDocumentStore
{
    ...
    public async Task<IEnumerable<string>> GetCollectionsAsync(CancellationToken cancellationToken = default)
    {
        ...
        // Use Concat().Distinct() over materialized collections to defer execution and avoid eagerly creating lists and O(N) .Contains() checks.
        return collections.Concat(_cappedCollections.Keys).Distinct();
    }
