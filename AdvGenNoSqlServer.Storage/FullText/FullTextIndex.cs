...
public class FullTextIndex : IFullTextIndex
{
    ...
    // Incrementally track total tokens to avoid O(N) recalculations
    ...
    {
        if (!_documents.TryRemove(documentId, out var existingInfo))
            return false;

        Interlocked.Add(ref _totalTokenCount, -existingInfo.TokenCount);
    ...
    public FullTextSearchResult Search(string query, FullTextSearchOptions? options = null)
    {
        ...
        try
        {
            ...
            // Hoist average document length calculation out of the loop
