...
public class FullTextIndex : IFullTextIndex
{
    ...
    // Tracks the sum of all document token counts incrementally to allow O(1) average calculations
    ...
    public FullTextSearchResult Search(string query, FullTextSearchOptions? options = null)
    {
        ...
        try
        {
            ...
            // Hoist average length calculation to avoid repeated O(N) evaluation inside inner loops
    ...
    private double GetAverageDocumentLength()
    {
        ...
        // O(1) performance using incrementally updated total token count
        return (double)Interlocked.Read(ref _totalTokenCount) / _documents.Count;
    }
