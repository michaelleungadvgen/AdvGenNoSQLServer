...
## 2024-05-18 - [Path Traversal bypass due to insufficient sanitization]
**Vulnerability:** Sanitization functions like `SanitizeFileName` that rely solely on `Path.GetInvalidFileNameChars()` do not prevent path traversal, because `.` (dot) is a valid character, permitting `..` sequences to bypass the check. This pattern existed in several places including AttachmentStore and GarbageCollectedDocumentStore.
**Learning:** Replacing invalid filename characters is insufficient for secure path combination. If the input allows valid directory names or dots, the combined path can still traverse outside the base directory.
**Prevention:** Always use `PathValidator.GetSafePath` to structurally enforce that the resulting path is rooted strictly within the intended base directory, regardless of character-level sanitization.
