...
## 2026-03-05 - [Path Traversal in Storage Layer]
**Vulnerability:** Path Traversal via unvalidated file paths constructed using `Path.Combine` with user inputs across `DatabaseManager.cs`, `GarbageCollectedDocumentStore.cs`, `AttachmentStore.cs`, and `DataExporter.cs`.
**Learning:** Even if `SanitizeFileName` is used, combining multiple directory segments with `Path.Combine` without a robust structural validation can lead to directory traversal vulnerabilities. `Path.Combine` natively respects prefix slashes or dots, enabling directory escape when base paths are incorrectly manipulated.
**Prevention:** Always use `PathValidator.GetSafePath` when constructing file paths involving user-controlled inputs, ensuring the resolved path strictly resides within the intended base directory.
