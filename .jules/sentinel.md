...
## 2026-03-05 - [Brute-force Vulnerability in AuthenticationManager]
**Vulnerability:** No rate limiting for authentication attempts in `AuthenticationManager.cs`. Vulnerable to brute-force attacks.
**Learning:** When implementing lockout or rate-limiting for authentication endpoints (e.g., in `AuthenticationManager`), always execute the computational verification step (such as PBKDF2 password hashing) even if the user is already locked out or doesn't exist. Bypassing the hash comparison for locked-out users creates a timing vulnerability that leaks lockout states or allows username enumeration. Furthermore, any class initializing a `Timer` must implement `IDisposable` (cascading to its instantiators and test `using` blocks) to prevent memory leaks. Finally, cleanup logic must remove all expired entries (including users with failed attempts below the lockout threshold) to prevent memory exhaustion.
**Prevention:** Implement failed attempt tracking using a `ConcurrentDictionary` and a background `Timer` for cleanup. Ensure constant-time password hashing is always executed, even for locked-out or non-existent users. Ensure `IDisposable` is implemented to properly dispose of the `Timer`.
