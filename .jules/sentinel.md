...
## 2026-03-05 - [Path Traversal in Database Management and Garbage Collection]
**Vulnerability:** Path Traversal via unvalidated inputs in `DatabaseManager.cs` and `GarbageCollectedDocumentStore.cs` allowing database creation or document paths to escape intended directories.
**Learning:** Raw `Path.Combine` operations involving user-controlled inputs (like database names or collection names) were used without verifying the final path boundaries, even though partial sanitization (`SanitizeFileName`) might have been applied to some elements. This is a recurring pattern across the storage layer.
**Prevention:** Systematically wrap all `Path.Combine` calls that construct file paths from user inputs with `PathValidator.GetSafePath` to ensure structural validation, preventing directory escape attacks via `../` combinations.
