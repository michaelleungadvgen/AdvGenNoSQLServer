...
**Vulnerability:** Path Traversal via unvalidated `name` parameter in `DatabaseManager.cs` allowing directory creation/deletion outside the intended base directory.
**Learning:** Even if slashes are filtered out, special characters like `..` can still be passed to traverse directories when used directly with `Path.Combine`. This can result in arbitrary directory deletion or creation.
**Prevention:** Always use `PathValidator.GetSafePath` when constructing file or directory paths from user-provided inputs to verify the bounds.
