...
## 2024-05-12 - [Fix Path Traversal in Multiple Storage Components]
**Vulnerability:** Path Traversal via unvalidated file paths constructed using `Path.Combine` without checking if the resulting path escaped the intended base directory. Affected components include `AttachmentStore`, `DatabaseManager`, `GarbageCollectedDocumentStore`, and `DataExporter`.
**Learning:** Concatenating user inputs using `Path.Combine` even after simple filename sanitization is insufficient, as it does not prevent directory traversal sequences like `../`. Malicious paths could read or overwrite files outside the intended base paths.
**Prevention:** Always wrap any constructed file path with `PathValidator.GetSafePath` which verifies that the normalized path strictly begins with the base path, thus thwarting traversal attacks.
