...
## 2024-07-01 - Prevent Path Traversal in Storage Layer
**Vulnerability:** User-controlled inputs in `Path.Combine` without proper bounds checking enabled path traversal attacks, allowing arbitrary file deletion and creation even with basic `SanitizeFileName` mechanisms in place.
**Learning:** Simple string sanitization (like replacing invalid chars or checking for `/`) is insufficient. Attackers can still bypass these checks or leverage existing directory structures. Path operations must always be validated against the resolved absolute paths.
**Prevention:** Always wrap any user-supplied path combination (`Path.Combine`) with `PathValidator.GetSafePath` systematically across the entire storage layer to ensure the resolved path strictly resides within the intended base directory.
