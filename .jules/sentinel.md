...
## 2023-11-20 - Systematically Prevent Path Traversal via PathValidator
**Vulnerability:** User-controlled values (e.g., collection names, document IDs, attachment names) were combined directly into file paths using `Path.Combine` across various storage classes (`DatabaseManager`, `GarbageCollectedDocumentStore`, `AttachmentStore`, `DataExporter`). Internal methods like `SanitizeFileName` are insufficient to reliably prevent directory traversal when combined directly.
**Learning:** `SanitizeFileName` relies on string replacement which is brittle and easily bypassed. Structural validation using a resolved base path is essential to guarantee that a path remains within its intended directory.
**Prevention:** Systematically wrap any user-controlled file path assembly (`Path.Combine`) with `PathValidator.GetSafePath(basePath, ...)` across the entire storage layer. Ensure that the first argument always explicitly dictates the intended sandbox directory constraint.
