...
## 2026-03-05 - [Path Traversal in Multiple Storage Services]
**Vulnerability:** Multiple path traversal vulnerabilities were found across `DatabaseManager.cs`, `GarbageCollectedDocumentStore.cs`, `Attachments/AttachmentStore.cs`, and `ImportExport/DataExporter.cs` where `Path.Combine` was used without validating if the resolved path stayed within the designated base directory.
**Learning:** Concatenating user inputs like collection names or document IDs using `Path.Combine` natively resolves parent directory markers (`..`). Without explicitly enforcing containment using an absolute path boundary validation method like `PathValidator.GetSafePath`, an attacker could escape storage roots and overwrite or read arbitrary files on the filesystem.
**Prevention:** Always wrap all external inputs combined into local file system paths via `Path.Combine` with structural validation using `PathValidator.GetSafePath` to safely constrain operations within allowed directory boundaries across the entire storage layer.
