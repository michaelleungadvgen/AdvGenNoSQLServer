...
## 2026-03-05 - [Brute Force Vulnerability in Authentication]
**Vulnerability:** Missing brute force protection on the authentication endpoint (`AuthenticationManager.Authenticate`). Attackers could attempt an unlimited number of login requests without being rate-limited or locked out.
**Learning:** Returning a simple `null` on failed authentication attempts without tracking failed attempts locally leaves the application vulnerable to automated brute force and dictionary attacks.
**Prevention:** Track failed login attempts locally (e.g. using a thread-safe `ConcurrentDictionary`). Enforce a temporary lockout (e.g. 15 minutes) when a user exceeds a maximum threshold (e.g. 5 attempts) to deter automated attacks. Additionally, always compute the verification step (e.g. password hash comparison) even when a user is locked out to prevent timing attacks from revealing the lockout state. Always implement periodic background cleanup for expired lockouts to prevent memory leaks in the tracker.
