...
**Vulnerability:** Path Traversal via unvalidated inputs in `DatabaseManager.cs`, `GarbageCollectedDocumentStore.cs`, `AttachmentStore.cs`, and `DataExporter.cs` allowing reads/writes outside the intended base directory.
**Learning:** `Path.Combine` is insufficient to prevent directory escape when combining user-controlled variables. Even internally generated components like `documentId` or `collectionName` might carry traversal payloads (`../`) from upstream vulnerabilities.
**Prevention:** Always wrap any user-controlled file path assembly (`Path.Combine`) with `PathValidator.GetSafePath` to ensure structural validation and prevent directory escape attacks across the entire storage layer.
